...
// Products shown under My Products on this branch
const subscribedIds = ["cuda", "nemo-llm", "dgx-cloud", "bionemo"]
...
// --- Insight Panel (shown inside Details drawer) ---
...
export function MyProducts() {
  ...
  // Dynamic filter tabs from subscribed products
  const filterLabels = [
    { value: "all", label: "All My Products" },
    ...subscribedIds.map(id => ({ value: id, label: getProductName(id) })),
  ]

  // Base pool: only subscribed products, minus dismissed and minus currently-watched actions
  ...
  // Watching list = initial placeholder items + actions user has watched
  ...
  function openDetails(action: ActionItem) {
    setWatchingOpen(false) // mutually exclusive
  ...
  function openWatching() {
    setSelectedAction(null) // mutually exclusive
  ...
  function handleWatch(action: ActionItem) {
    ...
    setTimeout(() => {
      ...
      // Trigger badge pulse — the card "lands" in the Watching bucket
  ...
  function handleUnwatch(itemId: string) {
    // If it's a watched action (not a placeholder), remove it from watched list
    if (watchedActionIds.includes(itemId)) {
      setWatchedActionIds(prev => prev.filter(id => id !== itemId))
    }
    // For placeholder items, silently drop (not persisted anyway — prototype only)
    // so the drawer list visually shortens
  }
    ...
    <div className="relative">
      <div className="flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-3">
          {/* Tab row with Watching button on the far right */}
